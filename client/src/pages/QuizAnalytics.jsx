    ...
    if (loading) {
        return (
            <div className="flex flex-col items-center justify-center min-h-[80vh]">
                ...
                <span className="text-slate-400 text-sm font-semibold tracking-wider uppercase">Loading Analytics Pipeline...</span>
            </div>
        );
    }
    ...
    if (error) {
        return (
            <div className="max-w-4xl mx-auto px-4 py-16 text-center">
                <div className="inline-flex p-4 bg-red-950/20 border border-red-500/30 text-red-400 rounded-full mb-4">
                    <svg className="w-8 h-8" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z" />
                    </svg>
                </div>
                <h2 className="text-2xl font-black text-white mb-2">Analytics Retrieval Failed</h2>
                <p className="text-slate-400 mb-6">{error}</p>
                <button
                    onClick={() => navigate("/dashboard")}
                    className="px-6 py-2.5 bg-slate-900 border border-slate-800 rounded-lg text-sm font-semibold text-slate-300 hover:text-white hover:bg-slate-950 transition-all duration-200"
                >
                    Return to Dashboard
                </button>
            </div>
    ...
    return (
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10 animate-fade-in-up">
            ...
            <div className="mb-8">
                <button
                    onClick={() => navigate("/dashboard")}
                    className="inline-flex items-center text-xs font-bold text-slate-400 hover:text-white transition-colors duration-200 mb-4 cursor-pointer group"
                >
                ...
                <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                    <div>
                        ...
                        <p className="mt-1 text-sm text-slate-400">
                            Quiz: <span className="text-white font-semibold">{quizDetails?.title || "Loading..."}</span>
                        </p>
                    </div>
                    <div className="px-4 py-2 bg-slate-950/40 border border-slate-800/80 rounded-xl text-xs font-medium text-slate-400 flex items-center space-x-2">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-10">
                ...
                <div className="glass-card p-6 flex items-center justify-between relative overflow-hidden group hover:border-cyan-500/25 transition-all duration-300">
                    <div>
                        <p className="text-xs font-bold text-slate-500 tracking-wider uppercase mb-1">Total Submissions</p>
                        <h3 className="text-4xl font-black text-white">{analytics?.totalAttempts || 0}</h3>
                        <p className="text-xs text-slate-400 mt-2">Completed student attempts</p>
                    </div>
                ...
                <div className="glass-card p-6 flex items-center justify-between relative overflow-hidden group hover:border-emerald-500/25 transition-all duration-300">
                    <div>
                        <p className="text-xs font-bold text-slate-500 tracking-wider uppercase mb-1">Class Average Score</p>
                    ...
                    <div className="p-3 bg-emerald-950/40 border border-emerald-800/30 text-emerald-400 rounded-xl">
                        <svg className="w-6 h-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                ...
                <div className="glass-card p-6 flex items-center justify-between relative overflow-hidden group hover:border-rose-500/25 transition-all duration-300">
                    <div>
                        <p className="text-xs font-bold text-slate-500 tracking-wider uppercase mb-1">Most Missed Concept</p>
                        <h3 className="text-2xl font-black text-white capitalize truncate max-w-[200px]" title={missedTag}>
                            {missedTag}
                        </h3>
                        <p className="text-xs text-slate-400 mt-2">
                    ...
                    <div className="p-3 bg-rose-950/40 border border-rose-800/30 text-rose-400 rounded-xl">
                        <svg className="w-6 h-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z" />
                        </svg>
                    </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                <div className="lg:col-span-1 flex flex-col gap-6">
                    <div className="glass-card p-6 h-full flex flex-col justify-between">
                        <div>
                            <h2 className="text-xl font-bold text-white mb-2">Adaptive Engine Triggers</h2>
                            <p className="text-xs text-slate-400 mb-6">
                                Distribution of student paths computed by the Phase 4 Adaptive Difficulty Engine.
                            </p>

                            {/* Proportional Stacked Bar Chart */}
                            <div className="h-10 w-full rounded-xl overflow-hidden flex mb-8 border border-slate-900 shadow-inner">
                                {analytics?.totalAttempts > 0 ? (
                                    ...
                                    <div className="w-full bg-slate-900 flex items-center justify-center text-xs text-slate-500">
                            ...
                            <div className="space-y-4">
                                {/* Enrichment Card */}
                                <div className="p-4 rounded-xl border border-teal-500/10 bg-teal-950/10 flex items-center justify-between">
                                    <div className="flex items-center space-x-3">
                                        <span className="w-3.5 h-3.5 rounded-full bg-emerald-500"></span>
                                        <div>
                                            <p className="text-sm font-bold text-white">Enrichment Path</p>
                                            <p className="text-[10px] text-slate-400">Scores &gt; 85% — Advanced deck</p>
                                        </div>
                                    </div>
                                    <div className="text-right">
                                        ...
                                        <p className="text-[10px] text-slate-500">{enrPercent}%</p>
                                    </div>
                                </div>

                                {/* Standard Card */}
                                <div className="p-4 rounded-xl border border-blue-500/10 bg-blue-950/10 flex items-center justify-between">
                                    <div className="flex items-center space-x-3">
                                        <span className="w-3.5 h-3.5 rounded-full bg-blue-500"></span>
                                        <div>
                                            <p className="text-sm font-bold text-white">Standard Path</p>
                                            <p className="text-[10px] text-slate-400">Scores 50% - 85% — Baseline deck</p>
                                        </div>
                                    </div>
                                    <div className="text-right">
                                        ...
                                        <p className="text-[10px] text-slate-500">{nonePercent}%</p>
                                    </div>
                                </div>

                                {/* Remediation Card */}
                                <div className="p-4 rounded-xl border border-amber-500/10 bg-amber-950/10 flex items-center justify-between">
                                    <div className="flex items-center space-x-3">
                                        <span className="w-3.5 h-3.5 rounded-full bg-amber-500"></span>
                                        <div>
                                            <p className="text-sm font-bold text-white">Remediation Path</p>
                                            <p className="text-[10px] text-slate-400">Scores &lt; 50% — Supportive deck</p>
                                        </div>
                                    </div>
                                    <div className="text-right">
                                        ...
                                        <p className="text-[10px] text-slate-500">{remPercent}%</p>
                                    </div>
                                </div>
                            </div>
                        </div>
                ...
                <div className="lg:col-span-2">
                    <div className="glass-card p-6">
                        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-6">
                            ...
                            <div className="relative w-full md:w-64">
                                <input
                                    ...
                                    className="w-full bg-slate-950/60 border border-slate-800 rounded-xl px-4 py-2 text-xs text-white placeholder-slate-500 focus:outline-none focus:border-cyan-500/50 transition-colors"
                                />
                        ...
                        {/* Matrix Filter Tabs */}
                        <div className="flex flex-wrap gap-2 mb-6 pb-4 border-b border-slate-800/60">
                            {[
                                ...
                                <button
                                    ...
                                    className={`px-3 py-1.5 rounded-lg text-xs font-bold transition-all duration-200 cursor-pointer ${questionFilter === tab.id
                                            ? "bg-cyan-950/40 text-cyan-400 border border-cyan-500/30 shadow-md"
                        ...
                        {filteredQuestions.length === 0 ? (
                            <div className="text-center py-16 border border-dashed border-slate-800 rounded-xl bg-slate-950/10">
                                ...
                                <p className="text-slate-500 text-xs">No questions found matching your filter criteria.</p>
                            </div>
                        ) : (
                                        ...
                                        >
                                            <div className="flex flex-col sm:flex-row justify-between items-start gap-3 mb-3">
                                                    ...
                                                    {q.tags && q.tags.length > 0 && (
                                                        <div className="flex flex-wrap gap-1.5 mt-2">
                                                            {q.tags.map((tag) => (
                                                                <span
                                                                    key={tag}
                                                                    className="px-2 py-0.5 rounded bg-slate-950/40 text-[10px] font-semibold text-slate-400 border border-slate-800/60 uppercase tracking-wider"
                                                                >
                                                                    {tag}
                                                                </span>
                                                            ))}
                                                ...
                                                <div className="text-right flex sm:flex-col items-center sm:items-end gap-2 shrink-0">
                                                    <span className={`px-2.5 py-0.5 rounded-full text-[10px] font-black uppercase tracking-wider ${tier.textClass} border ${tier.borderClass} bg-slate-950/40`}>
                                                        {tier.label}
                                                    </span>
